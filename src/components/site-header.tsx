  ...
  return (
    <header className="sticky top-0 z-20 border-b border-line bg-paper/90 backdrop-blur">
      <div className="mx-auto flex h-[68px] max-w-[1240px] items-center justify-between gap-6 px-4 sm:px-8">
        ...
        {session && (
          <nav className="hidden gap-5 text-[0.72rem] uppercase tracking-wider text-ink-soft lg:flex">
            {links.map((link) => (
        ...
        <div className="flex items-center gap-4">
          {session && (
            <span className="hidden text-[0.72rem] uppercase tracking-wider text-ink-faint xl:inline">
      ...
      {session && (
        <nav className="flex gap-5 overflow-x-auto border-t border-line px-4 py-2.5 text-[0.7rem] uppercase tracking-wider text-ink-soft lg:hidden">
          {links.map((link) => (
